...
def VoiceAssistant():
    ...
    with sr.Microphone() as Source:
        ...
        tk.Label(foot,text=vlenh).place(x=800, y=20)
...
def PreProcessing():
    ...
    cols = data_preprocessed.columns[(data_preprocessed.columns != '2018') & (
    ...
    X = data_preprocessed.loc[:, (data_preprocessed.columns != '2018') & (
        data_preprocessed.columns != 'Country Name') & (data_preprocessed.columns != 'ID_Name')]
    y = data_preprocessed[['2018']]
...
def Analyze():
    ...
    data_newa['TB theo z'] = data_preprocessed.select_dtypes(include='number').mean(axis=1)
...
def Predict():
    ...
    atr_predict_form.title('Chọn thuộc tính để dự đoán 2018')
    ...
    var3 = tk.IntVar()
    cols = ['Country Name']
    cb1 = tk.Checkbutton(atr_predict_form, text='1990',
                         variable=var1, onvalue=1, offvalue=0, command=lambda: cols.append('1990'))
    cb1.place(x=10, y=10)
    cb2 = tk.Checkbutton(atr_predict_form, text='2000',
                         variable=var2, onvalue=1, offvalue=0, command=lambda: cols.append('2000'))
    cb2.place(x=10, y=30)
    cb3 = tk.Checkbutton(atr_predict_form, text='2017',
                         variable=var3, onvalue=1, offvalue=0, command=lambda: cols.append('2017'))
    cb3.place(x=10, y=50)
    btnSubmit = tk.Button(atr_predict_form, text='Dự đoán', width=10, height=2,
                          command=lambda: SubmitPredict(cols, atr_predict_form))
    btnSubmit.place(x=180, y=25)

def SubmitPredict(cols, form):
...
def DropColumns(columns, data, chooseAtr_form):
    if len(columns) == len(data.columns):
        ...
        chooseAtr_form.focus()
        return
    if 'Country Name' in columns or '2018' in columns:
        if 'Country Name' in columns and '2018' in columns:
            messagebox.showwarning(
                "Thông báo", "Không chọn Country vs 2018 ")
            chooseAtr_form.focus()
            return
        if 'Country Name' in columns:
            messagebox.showwarning("Thông báo", "Không chọn Country ")
            chooseAtr_form.focus()
            return
        if '2018' in columns:
            messagebox.showwarning("Thông báo", "Không được drop 2018")
            chooseAtr_form.focus()
            return
...
def ExportCSV():
    ...
    try:
        ...
        path = f'./CSVFolder/Processed_CO2Emission_{current_time}.csv'
...
btnPredict = tk.Button(foot, text='5.Dự Đoán năm 2018', width=15, height=2, state=tk.DISABLED, command=Predict)
...
btnClose = tk.Button(foot, text='8.Thoát', width=10, height=2, command=Close)
