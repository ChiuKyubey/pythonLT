...
pygame.display.set_caption('69 NGUYEN KHANG THINH: Game ĐUA XE')
...
OBSTACLESSPEED = 2
CHANGESPEED = 0.001
OBSTACLESIMG = pygame.image.load('imgs/obstacles.png')
...
class Obstacles():
    ...
    def draw(self):
        for i in range(5):
            x = int(X_MARGIN + self.ls[i][0] * LANEWIDTH +
                    (LANEWIDTH - self.width) / 2)
            y = int(self.ls[i][1])
...
class Score():
    ...
    def draw(self):
        ...
        scoreSuface = font.render('Score: ' + str(int(self.score)), True,
                                  (0, 0, 0))
...
def rectCollision(rect1, rect2):
    if rect1[0] <= rect2[0] + rect2[2] and rect2[
            0] <= rect1[0] + rect1[2] and rect1[
                1] <= rect2[1] + rect2[3] and rect2[1] <= rect1[1] + rect1[3]:
        return True
    return False
...
def isGameover(car, obstacles):
    ...
    for i in range(5):
        x = int(X_MARGIN + obstacles.ls[i][0] * LANEWIDTH +
                (LANEWIDTH - obstacles.width) / 2)
...
def gameOver(bg, car, obstacles, score):
    ...
    while True:
        ...
        obstacles.draw()
        score.draw()
        DISPLAYSURF.blit(headingSuface, (int(
            (WINDOWWIDTH - headingSize[0]) / 2), 100))
        DISPLAYSURF.blit(commentSuface, (int(
            (WINDOWWIDTH - commentSize[0]) / 2), 400))
